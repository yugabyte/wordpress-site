...
class WPSEO_Redirect_Page_Presenter implements WPSEO_Redirect_Presenter {
	...
	private function get_tab_presenter( $tab_to_display ) {
		...
		switch ( $tab_to_display ) {
			case WPSEO_Redirect::FORMAT_PLAIN:
			case WPSEO_Redirect::FORMAT_REGEX:
	...
	private function navigation_tabs( $current_tab ) {
		$tabs = array(
			WPSEO_Redirect::FORMAT_PLAIN => __( 'Redirects', 'wordpress-seo-premium' ),
			WPSEO_Redirect::FORMAT_REGEX => __( 'Regex Redirects', 'wordpress-seo-premium' ),
		);
